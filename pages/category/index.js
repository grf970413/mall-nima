...
Page({
    ...
    getCatalog: function() {
         ...
         * 查询商品分类列表
         */
        util.request("http://localhost:9002/product/getAllProductCategory").then(function(res) {
            that.setData({
                navList: res.data
            });
        });
    ...
    getCurrentList: function(id) {
        let that = this;
        util.request("http://localhost:9002/product/listProduct", {
            pageSize: that.data.size,
            pageNum: that.data.allPage,
            id: id
